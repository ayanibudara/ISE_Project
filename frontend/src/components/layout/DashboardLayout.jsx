...
const DashboardLayout = ({ children }) => {
  ...
  if (user?.role === "Tourist") {
    ...
    navigation.push(
     ...
     // { name: 'Manage Package', href: '/Services/ManagePackages', icon: (
    ...
    navigation.push(
      ...
      { name: 'Guide Scheduling', href: '/dashboard/admin/guide-scheduling', icon: (
      <svg className="mr-4 h-6 w-6 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <nav className="space-y-2">
                  {navigation.map((item) => (
                    <Link
                      ...
                      `}
                    >
                      <div className="p-2 rounded-lg mr-3 bg-slate-700/50 text-slate-400 group-hover:bg-slate-600/50 group-hover:text-white transition-colors duration-200">
                        {item.icon}
                      </div>
